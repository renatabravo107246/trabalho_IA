...
+paciente(_,Nome,Data,Sexo,Morada) ::
    ( \+ interdito(Nome),
      \+ interdito(Data),
      \+ interdito(Sexo),
      \+ interdito(Morada)
...
+consulta(_,_,Pac,_,_,_,_) ::
    ( paciente(Pac,_,_,_,_) ,
      \+ (-paciente(Pac,_,_,_,_)),
      \+ interdito(Pac)
    ).

+consulta(_,(_,_,AnoC), Pac, _, _, _, _) ::
    ( paciente(Pac,_,(_,_,AnoN),_,_),
      AnoC >= AnoN,
      AnoC =< 2100
    ).

% Idade plausível
+consulta(_,(_,_,_),_,Idade,_,_,_):: (Idade>=0, Idade=<115).

% Idade coerente com a data de nascimento do paciente
+consulta(IdC, (DiaC, MesC, AnoC), Pac, Idade, _, _, _) ::
    ( paciente(Pac, _, (DiaN, MesN, AnoN), _, _),
      AnoC - AnoN - ((MesC < MesN ; (MesC == MesN, DiaC < DiaN)) -> 1 ; 0) =:= Idade
    ).

% Mes válido
+consulta(_,(_,M,_),_,_,_,_,_):: (M>=1, M=<12).

% dias válidos (meses com 31 dias)
(+consulta(_,(D,M,_),_,_,_,_,_)::(D>=1, D=<31)):- pertence(M,[1,3,5,7,8,10,12]).
% dias válidos (meses com 30 dias)
...
% Um paciente não pode marcar 2 consultas no mesmo dia 
+consulta(_, (D,M,A), Pac, _, _, _, _) ::
    ( findall((D,M,A), consulta(_, (D,M,A), Pac, _, _, _, _), L),
      comprimento(L, N),
      N =< 1 ).

% ano da consulta tem de ser depois do ano de nascimento do paciente
+consulta(_,(_,_,AnoC), Pac, _, _, _, _) ::
    ( paciente(Pac,_,(_,_,AnoN),_,_),
      AnoC >= AnoN,
      AnoC =< 2100
...
% ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
% ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
% --- Involucao da Base de Conhecimento ------------------------------------------------------------------------------------------------------------------------------------
% ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
